...
void i2c_read_nack(); // read incoming NACK
...
ISR(USART0_RX_vect){
	...
	switch(x){
		case 'a':
		data = x; i2c_start(); pieper_uit(); break;
		case 'w':
		data = x; i2c_start(); pieper_uit(); break;
		case 'd':
		data = x; i2c_start(); pieper_uit(); break;
		case 's':
		data = x; i2c_start(); pieper_aan(); break;
		case '1':
		data = x; i2c_start(); break;
		case '2':
		data = x; i2c_start(); break;
		case '3':
		data = x; i2c_start(); break;
...
ISR(TWI_vect){
	...
	switch(TWI_STATUS){
		...
		i2c_send_byte((SLA << 1) + 0);
		
		case 0x18: // SLA+W ACK has been transmitted; ACK has been received
		i2c_send_byte(data); // <------ USART interrupt value?
		break;
		...
		i2c_send_byte((SLA << 1) + 1);
		break;
		
		case 0x40: // SLA + R has been transmitted; ACK has been received
		case 0x58: // Data byte has been received; NOT ACK has been returned
		while (~UCSR0A & (1 << UDRE0));
		UDR0 = TWDR; // print data; 
		break;
...
void i2c_start(void) {
	TWCR = (1 << TWINT) | (1 << TWSTA) | (1 << TWEN) | (1 << TWIE); // transmit START condition
	//while ( !(TWCR & (1 << TWINT)) ); // wait for end of transmission
}

void i2c_send_byte(uint8_t data) {
	TWDR = (data);
	TWCR = (1 << TWINT) | (1 << TWEN) | (1 << TWIE); // start transmission
	//while ( !(TWCR & (1 << TWINT)) ); // wait for end of transmission
}
